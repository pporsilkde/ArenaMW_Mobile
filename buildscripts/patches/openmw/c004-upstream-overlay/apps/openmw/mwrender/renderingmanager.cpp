...
namespace MWRender
{
    ...
    void RenderingManager::updateProjectionMatrix()
    {
        ...
        if (mShadowManager)
        {
            // V17: the old mobile optimizer multiplied shadow range by 0.25, so a
            // requested 8192 became only 2048 at an 8192 camera distance (or 4096
            // with a 16384 camera). Respect the user's shadow-distance setting
            // directly. It only needs to be capped by the mobile safety limit and
            // by the camera far plane because nothing beyond that plane is visible.
            const float configuredShadowDistance = std::clamp(
                Settings::Manager::getFloat("maximum shadow map distance", "Shadows"), 0.f, 8192.f);
            const float shadowDistance = std::min(configuredShadowDistance, mViewDistance);
            mShadowManager->setMaximumShadowMapDistance(shadowDistance);
        }
    }
