...
void Water::updateWaterMaterial()
{
    ...
    // Android V17.1 keeps two supported modes: Off and stable legacy/simple.
    // Old New/PBR values are intentionally rendered through the Simple path.
    std::string shaderWaterMode = Settings::Manager::getString("shader mode", "Water");
    Misc::StringUtils::lowerCaseInPlace(shaderWaterMode);
    const bool shaderWaterEnabled = shaderWaterMode != "off" && shaderWaterMode != "false"
        && shaderWaterMode != "0" && shaderWaterMode != "disabled";
    const bool pbrWaterEnabled = false;
