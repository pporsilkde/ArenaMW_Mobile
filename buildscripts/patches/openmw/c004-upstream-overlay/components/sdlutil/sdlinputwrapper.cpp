...
InputWrapper::InputWrapper(SDL_Window* window, osg::ref_ptr<osgViewer::Viewer> viewer, bool grab) :
    ...
    void InputWrapper::handleWindowEvent(const SDL_Event& evt)
    {
        switch (evt.window.event) {
            ...
            {
                // Use the real OpenGL drawable size, not SDL's logical window
                // size. This matters on Windows with DPI scaling and especially
                // when an exclusive-fullscreen Intel context is minimized and
                // restored. During Alt+Tab SDL may temporarily expose a 0x0
                // drawable; feeding that transient size into OSG can leave the
                // swapchain/viewport oscillating after the window returns.
                int w = 0;
                int h = 0;
