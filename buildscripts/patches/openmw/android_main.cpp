...
extern "C" int Java_org_libsdl_app_SDLActivity_nativeInit(JNIEnv* env, jclass cls, jobject obj) {
    ...
    // SDL_HINT_RENDER_DRIVER / SDL_HINT_FRAMEBUFFER_ACCELERATION нарочно НЕ задаём:
    // NG-GL4ES работает как GLES3 (см. LIBGL_ES=3 / OPENMW_GLES_VERSION=3 в GameActivity),
    // а форс "opengles2" заставлял SDL создавать GLES2-контекст — в результате шейдеры
    // 300 es / sampler2DShadow / UBO не исполнялись корректно и давали артефакты.
