 ...
 */
            case COMMAND_TEXTEDIT_HIDE:
                if (mTextEdit != null) {
                    ...
                    boolean hidden = GameActivity.hideOsk();
                    if (!hidden) {
...
*/
    ...
    static class ShowTextInputTask implements Runnable {
        ...
        public void run() {
            ...
            boolean shown = GameActivity.showOsk();
            InputMethodManager imm = (InputMethodManager) SDL.getContext().getSystemService(Context.INPUT_METHOD_SERVICE);
            if (!shown) {
                if (imm != null) {
                ...
                // GameActivity берёт ввод на себя (угловая кнопка + собственный OSK),
                // но requestFocus() на DummyEdit мог триггернуть системную клавиатуру.
                // Убираем её — пользователь сам решит, открывать ли наш OSK.
                if (imm != null) {
                    imm.hideSoftInputFromWindow(mTextEdit.getWindowToken(), 0);
                }
            }

            mScreenKeyboardShown = shown;
        }
    }
...
class DummyEdit extends View implements View.OnKeyListener {
    ...
    public InputConnection onCreateInputConnection(EditorInfo outAttrs) {
        ...
        // Намеренно не используем TYPE_TEXT_VARIATION_VISIBLE_PASSWORD:
        // с этим флагом Android считает поле «секретным» и блокирует снятие
        // скриншотов (а также запись экрана) пока открыта клавиатура. В игре
        // ничего секретного не вводится — используем обычный текст.
